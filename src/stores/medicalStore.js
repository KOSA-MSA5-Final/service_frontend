    ...
    actions: {
        ...
        async fetchMedicalRecordsByProfileId(profileId) {
            ...
            try {
                ...
                const response = await axios.get(`https://mgng.site/api/medical/profile/${profileId}`, {
                    headers: {
        ...
        async fetchMedicalRecordById(medicalId) {
            ...
            try {
                const token = localStorage.getItem('token'); // 토큰을 가져옴
                const response = await axios.get(`https://mgng.site/api/medical/${medicalId}`, {
                    headers: {
                        Authorization: `Bearer ${token}`, // 토큰을 헤더에 추가
                    },
                });
        ...
        async fetchLatestMedicalRecordByProfileId(profileId) {
            ...
            try {
                const token = localStorage.getItem('token'); // 토큰을 가져옴
                const response = await axios.get(`https://mgng.site/api/medical/profile/${profileId}/latest`, {
                    headers: {
                        Authorization: `Bearer ${token}`, // 토큰을 헤더에 추가
                    },
                });
